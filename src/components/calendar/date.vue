    ...
    methods: {
        init(){
            ...
            if (this.value.length>0) {
                if (this.range) { //范围
                    ...
                    this.month = parseInt(this.value[0][1]) - 1  //原本
                    // this.month = parseInt(this.value[0][1])   //修改
        ...
        select(k1, k2, e) {
            ...
            if (this.range) {
                ...
                let _rangeEnd = [this.year, this.month, this.days[k1][k2].day+this.selectnum-1]
                this.rangeEndTemp = 0
                let lastDateOfMonth = new Date(this.year, this.month + 1, 0).getDate()    //当月最后一天
                let lastDayOfLastMonth = new Date(this.year, this.month, 0).getDate()     //最后一月的最后一天
                console.log(`当月最后一天
                ${lastDateOfMonth}`)
                console.log(`最后一月的最后一天
                ${lastDayOfLastMonth}`)
                // 如果可选天数超过当前月份天数，需设置新的日期格式
                if(this.days[k1][k2].day+this.selectnum-1 > lastDateOfMonth){
                    console.log(`超过啦，需要重置日期格式`)
                    // 如果是最后一个月份
                    if(this.month == 11){
                        _rangeEnd = [this.year+1, 0, this.days[k1][k2].day+this.selectnum-1 - lastDateOfMonth]
                    }else{
                        _rangeEnd = [this.year, this.month+1, this.days[k1][k2].day+this.selectnum-1 - lastDateOfMonth]
                    }
                }
